	...
	<div class="row">
		...
		<div class="col-md-8">
			<table class="manage-table responsive-table">
				<tr>
					<th><i class="fa fa-file-text"></i> Rent Properties</th>
					<th class="expire-date"><i class="fa fa-calendar"></i> Posted Date</th>
					<th><i class="fa fa-anchor"></i> Action</th>
				</tr>
				<%
				System.out.println(session.getAttribute("uid"));
				PreparedStatement ps= DbConnect.getCon().prepareStatement("SELECT * FROM tblrent_properties WHERE Owner_Id = '"+session.getAttribute("uid")+"'");
					...
					<tr>
						<td class="title-container">
							...
							<div class="title">
								...
								<span><%= rs.getString(17) %>,&nbsp;<%= rs.getString(19) %>&nbsp;-&nbsp;<%= rs.getString(20) %>, <%= rs.getString(18) %></span>
								<span class="table-property-price"><%= rs.getString(12) %></span>
							</div>
						</td>
						
						<td class="expire-date"><%= rs.getString(16) %></td>
						
						<td class="action">
							<a href="myproprm.jsp?PropRentId=<%= rs.getString(2) %>" onclick="return confirm('Are you ssure?')" class="delete"><i class="fa fa-remove"></i> Delete</a>
						</td>
					</tr>
				<%} %>
			</table>
