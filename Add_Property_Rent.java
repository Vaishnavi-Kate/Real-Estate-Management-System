...
@WebServlet({"/Add_Property_Rent"})
public class Add_Property_Rent
  extends HttpServlet
{
  private static final long serialVersionUID = 1L;
  
  public Add_Property_Rent() {}
    ...
    throws ServletException, IOException
  {
    ...
    try
    {
      String sql = "insert into tblrent_properties(Owner_Id,Property_Name, Property_Category, Property_Type, Floor, Builtup_Area, No_Of_Bathrooms, Furnishing, Parking, Age_Of_Property, Tenant_Type, Deposit, Rent, Availability, AvailableDate, Address, State, City, Pincode, Posted_Date,Lift, Borewell, Generator, Playground, Balconies,Description,Solar) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,curdate(),?,?,?,?,?,?,?)";
      ...
      if (check > 0)
      {
        ...
        out.print("<script>");
        out.print("alert('Successfully Inserted Details..'); location.href='Upload_Add_Property_Rent_Images.jsp'; ");
        out.print("</script>");
      }
      else
      {
        out.print("<script>");
        out.print("alert('Try Later'); location.href='AddProperty_Rent.jsp'; ");
        out.print("</script>");
      }
    }
    catch (Exception e)
