...
@WebServlet({"/Add_Property_Flatmate"})
public class Add_Property_Flatmate
  extends HttpServlet
{
  private static final long serialVersionUID = 1L;
  
  public Add_Property_Flatmate() {}
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
  {
    ...
    try
    {
      String sql = "insert into tblflatmate_properties(Owner_Id,Property_Name, Property_Category, Property_Type, Floor, Builtup_Area, No_Of_Bathrooms, Furnishing, Parking, Age_Of_Property, Tenant_Type, Deposit, Rent, Availability, AvailableDate, Address, State, City, Pincode, Posted_Date,Lift, Borewell, Generator, Playground, Balconies,Description,Solar) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,curdate(),?,?,?,?,?,?,?)";
      ...
      ps.setString(12, deposit);
      ps.setString(13, rent);
      ps.setString(14, avail);
      ps.setString(15, availability_date);
      ps.setString(16, address);
      ps.setString(17, state);
      ps.setString(18, city);
      ps.setString(19, pincode);
      ps.setString(20, Lift);
      ps.setString(21, Borewell);
      ps.setString(22, Generator);
      ps.setString(23, Playground);
      ps.setString(24, Balconies);
      ps.setString(25, desc);
      ps.setString(26, Solar);
      ...
      if (check > 0)
      {
        ...
        out.print("<script>");
        out.print("alert('Successfully Inserted Details..'); location.href='Upload_Add_Property_Flatmate_Images.jsp'; ");
        out.print("</script>");
      }
      else
      {
        out.print("<script>");
        out.print("alert('Try Later'); location.href='Add_Property_Flatmate.jsp'; ");
        out.print("</script>");
      }
    } catch (Exception e) {
      e.printStackTrace();
